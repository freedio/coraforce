\ Forcembler vocabulary for GForth Linux-4.19 amd64
...
: rex, ( -- )
  rex? fpuop? andn if rex@ 8 = mmxop? and unless rex@ mmxop? 8 and andn $40 u+ tc, then then ;
...
: logarop-immacc  ( imm acc -- )  2 !args !opsize= size>immsize .size #WORD < 5 + A@ 8* u+ nip op, ;  \ unused
: logarop-immr/m  ( imm r/m -- )  2 !args !opsize  opsizext $80 + swap  A@ r/mreg! nip op,  ;
: logarop-imm ( imm r/m -- )  2 !args dup accu? if  logarop-immacc  else  logarop-immr/m  then ;  \ unused
...
: logarop ( ... opc )
  create c,
  does>  c@ >A 2dup imm>r/m? if logarop-immr/m else 2dup reg>r/m? if logarop-reg else 2dup mem>reg? if
...
: jmpcall-imm ( opc imm )
  drop imm@ toff - -126 130 within near? andn if  dup if  jmp-rel8 exit  then  then
  imm@ swap addrwidth@ DWORD min dup 1+  toff ( 4+ ) + reladdr $E8 + op&,
...
: (UNTIL) ( [a] cond -- )  32 assertCodeSpace  A@ back dup toff 3 + - -128 128 within simblock? andn if
  # swap 1 [xor] LIKELY ?JMP  else >A 1 skip 1+ # swap UNLIKELY ?JMP  A> # NEAR JMP then  A> endblock ;
: (ASLONG) ( [a] cond -- )  32 assertCodeSpace  A@ back dup toff 3 + - -128 128 within simblock? andn if
...
: (FOR) ( -- [a] )  (BEGIN) ;
: (NEXT) ( [a] -- )  32 assertCodeSpace  A@ back dup toff 2+ - -128 128 within simblock? andn if
  # LOOP  else  >A  RCX DEC  1 skip # 0= ?JMP  A> # NEAR JMP  then  A> endblock ;
: (NEXTIF) ( [a] -- )  32 assertCodeSpace  A@ back dup toff 2+ - -128 128 within simblock? andn if
  # swap ?LOOP  else  CONDLOOP_RANGE$ error  then  A> endblock ;
: (BREAK) ( [a] -- [a] )  A@ # JMP ;
: (BREAKLOOP) ( [a] -- [a] )  A> A@ # JMP >A ;
: (CONTINUE) ( [a] -- [a] )  A@ T.ADDRSIZE 1+ + # JMP ;
: (?BREAK) ( [a] cond -- [a] )  A@ dup toff 2+ - -128 128 within simblock? andn if
  # swap UNLIKELY ?JMP  else  >A 1 skip 1+ # swap 1 [xor] LIKELY ?JMP  A> # NEAR JMP  then ;
: (?CONTINUE) ( [a] cond -- [a] )  A@ T.ADDRSIZE 1+ + dup toff 2+ - -128 128 within simblock? andn if
  # swap UNLIKELY ?JMP  else  >A 1 skip 1+ # swap 1 [xor] LIKELY ?JMP  A> # NEAR JMP  then ;
