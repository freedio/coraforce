...
3 constant ENTER#                                     ( Number of bytes in the ENTER sequence )
4 constant EXIT#                                      ( Number of bytes in the EXIT sequence )
...
: ?ISZERO, ( x -- )  RAX RAX TEST  RAX POP  0= ;
: ?ISNOTZERO, ( x -- )  RAX RAX TEST  RAX POP  0≠ ;
: ?ISNEGATIVE, ( x -- )  RAX RAX TEST  RAX POP  0< ;
: ?ISNOTNEGATIVE, ( x -- )  RAX RAX TEST  RAX POP  0≥ ;
: ?ISPOSITIVE, ( x -- )  RAX RAX TEST  RAX POP  0> ;
: ?ISNOTPOSITIVE, ( x -- )  RAX RAX TEST  RAX POP  0≤ ;

: ?ISEQUAL, ( x1 x2 -- )  RDX POP  RAX RDX CMP  RAX POP  = ;
: ?ISNOTEQUAL, ( x1 x2 -- )  RDX POP  RAX RDX CMP  RAX POP  ≠ ;
: ?ISLESS, ( n1 n2 -- )  RDX POP  RAX RDX CMP  RAX POP  < ;
: ?ISNOTLESS, ( n1 n2 -- )  RDX POP  RAX RDX CMP  RAX POP  ≥ ;
: ?ISGREATER, ( n1 n2 -- )  RDX POP  RAX RDX CMP  RAX POP  > ;
: ?ISNOTGREATER, ( n1 n2 -- )  RDX POP  RAX RDX CMP  RAX POP  ≤ ;
: ?ISBELOW, ( u1 u2 -- )  RDX POP  RAX RDX CMP  RAX POP  U< ;
: ?ISNOTBELOW, ( u1 u2 -- )  RDX POP  RAX RDX CMP  RAX POP  U≥ ;
: ?ISABOVE, ( u1 u2 -- )  RDX POP  RAX RDX CMP  RAX POP  U> ;
: ?ISNOTABOVE, ( u1 u2 -- )  RDX POP  RAX RDX CMP  RAX POP  U≤ ;
: ?DUPIF, ( x -- [x] )  RAX RAX TEST  0- IFLIKELY  RAX PUSH  THEN  0≠ ;

: ?ISEQUAL#, ( x1 -- )  RAX POP  = ;
: ?ISNOTEQUAL#, ( x1 -- )  RAX POP  ≠ ;
: ?ISLESS#, ( x1 -- )  RAX POP  < ;
: ?ISNOTLESS#, ( x1 -- )  RAX POP  ≥ ;
: ?ISGREATER#, ( x1 -- )  RAX POP  > ;
: ?ISNOTGREATER#, ( x1 -- )  RAX POP  ≤ ;
: ?ISBELOW#, ( x1 -- )  RAX POP  U< ;
: ?ISNOTBELOW#, ( x1 -- )  RAX POP  U≥ ;
: ?ISABOVE#, ( x1 -- )  RAX POP  U> ;
: ?ISNOTABOVE#, ( x1 -- )  RAX POP  U≤ ;
...
: DECSTOREC, ( a c -- a-1 )  RDX POP  1 # RDX SUB  AL 0 [RDX] MOV  RDX RAX MOV ;
...
( Formats string a$ with u arguments ..., prints it to stderr with a leading cr, and aborts the process. )
